...
# -------- افزودن واریانس (نویز) به ستون‌های عددی --------
noise_std_ratio = 0.25  # نویز ۵٪ انحراف معیار ستون

numeric_cols = ['depth', 'porosity', 'permeability', 'pressure', 'temperature', 
                ...
                'rpm', 'flow_rate', 'ecd', 'overbalance', 'perforation_density']

for col in numeric_cols:
    noise = np.random.normal(0, noise_std_ratio * df[col].std(), size=df.shape[0])
    df[col] += noise

# اصلاح محدوده ستون‌های حساس
df['porosity'] = df['porosity'].clip(4, 8)
df['permeability'] = df['permeability'].clip(50, 500)
df['ph'] = df['ph'].clip(6, 9)
df['mud_weight'] = df['mud_weight'].clip(10, 15)
df['mud_viscosity'] = df['mud_viscosity'].clip(30, 60)
df['salinity'] = df['salinity'].clip(5000, 50000)
df['wob'] = df['wob'].clip(5000, 25000)
df['rpm'] = df['rpm'].clip(50, 150)
df['ecd'] = df['ecd'].clip(10, 14)
df['overbalance'] = df['overbalance'].clip(200, 800)
df['perforation_density'] = df['perforation_density'].clip(4, 12)

# تبدیل برخی ستون‌های عددی به float32 برای صرفه‌جویی حافظه
for col in numeric_cols:
    df[col] = df[col].astype(np.float32)

# ذخیره به فایل پارکت
df.to_parquet("formation_damage_dataset.parquet", index=False)
print("✅ فایل formation_damage_dataset.parquet با واریانس اضافه شده ذخیره شد.")
